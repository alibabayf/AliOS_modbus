...
static int check_spiffs_fs_interface()
{
	int ret, fd;
	const char *string = "test if spiffs fs mount ok";
    char testfile[64] = {0};

    snprintf(testfile,sizeof(testfile),"%s/%s",BE_FS_ROOT_DIR,"testfile.txt");
	fd = be_osal_open(testfile, O_RDWR | O_CREAT | O_TRUNC);
	if (fd <= 0) {
		be_warn(JS_FS_TAG,"check_spiffs_fs_interface open fail\n\r");
		return 0;
	}
	
	ret = be_osal_write(fd, string, strlen(string));
	if (ret <= 0){
		be_warn(JS_FS_TAG,"check_spiffs_fs_interface write fail\n\r");
		return 0;
	}
	
	ret = be_osal_unlink(testfile);
	if (ret){
		be_warn(JS_FS_TAG,"check_spiffs_fs_interface sync fail\n\r");
		return 0;
	} 
...
{
	int ret = -1;
	int fd = 0,len = 0,size = 0;
	be_jse_symbol_t *arg0 = NULL,*ret_jse_symbol = NULL;
    ...
    len = be_osal_read(fd, p_read_buff, size);
    if( len > 0 ){
        p_read_buff[len] = 0;
    }
    be_osal_close(fd);
...
static be_jse_symbol_t* module_fs_write()
{
	...
	if ( arg0 == NULL || !symbol_is_string(arg0) 
		...
		|| arg2 == NULL || !symbol_is_string(arg2)
	...
	int fd = be_osal_open(path, convert_fs_mode_to_oflag(mode));
	if (fd < 0) {
		be_error(JS_FS_TAG,"be_osal_open fail\n\r");
		goto done;
	}

	len= be_osal_write(fd, str, strlen(str));
	if (len <= 0){
...
/*****************************************************************************
*Function:    module_fs_rename
*Description: js native addon for FILE.rename(filepath)
			  rename the file
*Called by:   js api
*Input:       filepath : string 
*Output:      0 rename ok ;other rename fail
*****************************************************************************/
static be_jse_symbol_t* module_fs_rename()
{
	be_jse_symbol_t *arg0 = NULL;
	be_jse_symbol_t *arg1 = NULL;
	int ret = -1;
	char old_path[BE_FS_PATH_LEN_MAX] = {0};
	char new_path[BE_FS_PATH_LEN_MAX] = {0};

	be_jse_handle_function(0, &arg0,&arg1,NULL,NULL);
	if ( (arg0 == NULL) || !symbol_is_string(arg0) 
		 || (arg1 == NULL) || !symbol_is_string(arg1) ){
		be_error(JS_FS_TAG,"module_fs_rename param invalid\n\r");
		goto done;
	}

	symbol_to_str(arg0, old_path, symbol_str_len(arg0));
	symbol_to_str(arg1, new_path, symbol_str_len(arg1));
	
	ret = be_osal_rename(old_path,new_path);
											  ...
											  const char* name) 
{
	...
	if (strcmp(name,"rename") == 0) return module_fs_rename();
	
	return BE_JSE_FUNC_UNHANDLED;
}

											  									  
void module_fs_load(void)
{
	int ret = -1;
