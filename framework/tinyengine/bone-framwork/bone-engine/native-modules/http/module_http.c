...
#define ASSERT_NULL_JUMP(ptr) if (ptr == NULL) { printf("%s,%d\n\r",__FUNCTION__,__LINE__); goto done;}
...
static int http_send(int sockfd,char *buffer)
{
	...
	int size = strlen(buffer);
     ...
     while (size > 0) {
        ...
        //printf("Bytes sent: %ld", bytes_sent);
    }
...
static int http_recv(int sockfd,char *buffer)
{
    ...
    while (total_received < HTTP_BUFF_SIZE) {
        bytes_received = recv(sockfd, buffer, (HTTP_BUFF_SIZE-total_received), 0);
...
static int http_request(char *url,char *http_buffer)
{
	int socketid  = -1;
	int ret = -1;
...
static void task_http_request_fun(void * arg)
{
	...
	async->params[0] = new_str_symbol(http_buf);
	
	ret = be_osal_schedule_call(be_jse_async_event_cb, async);
	if( ret >= 0 ) {
		INC_SYMBL_REF(async->func);
	}else {
		be_error(JS_HTTP_TAG, "fatal error for be_osal_schedule_call\n\r");
	}
	...
	be_debug(JS_HTTP_TAG,"aos get task name=%s\n\r",be_osal_task_name());
	be_osal_exit_task(0);
}
...
*Input:       url: STRING object like http://www.baidu.com
			  functon(data) is a function object which is the callback
			  data is String Object which is the result http request,
			  when data is "ERROR" means http request error
*Output:      None
*****************************************************************************/
...
static be_jse_symbol_t *module_http_request()
{
	...
	char *url = NULL, *http_buffer = NULL;
	...
	http_buffer = calloc(1,HTTP_BUFF_SIZE);
	if (!http_buffer){
		free(url);
		goto done;
	}
	...
	INC_SYMBL_REF(arg1);
	be_osal_new_task("http_request",task_http_request_fun,schedule_msg,4096,NULL);
