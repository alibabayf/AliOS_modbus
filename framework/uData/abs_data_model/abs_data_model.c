...
#define DEFAULT_TIMER_INTERVAL 3000 /* 1000ms */
...
sensor_node_t g_sensor_node[] = {
    ...
    { TAG_DEV_TODAYRAIN, dev_todayrainfall_path, 0 },{ TAG_DEV_HALL, dev_hall_path, 0 }, 
    { TAG_DEV_HR, dev_hr_path, 0 }, { TAG_DEV_GPS, dev_gps_path, 0 },  
};
...
static void abs_data_post_timer_expried_dev(uint64_t timestamp)
{
    ...
    for (i = 0; i < abs_data_cnt; i++) {
        ...
        if ((timestamp - g_abs_data_db[i]->cur_timestamp) >=
            ...
            LOG("yftest g_abs_data_db[i]->interval =%d\n",g_abs_data_db[i]->interval);
...
{
    if (unlikely(tag >= TAG_DEV_SENSOR_NUM_MAX)) {
        return NULL;
    }
...
int abs_data_open(uData_service_t *service)
{
    ...
    LOG("yftest abs_data_create_obj_ctx interval=%d\n",interval);
...
int abs_data_read(sensor_tag_e tag, void *pdata, uint32_t nbyte)
{
    ...
    LOG("yftest abs_data_read tag =%d\n",tag);
    ...
    LOG("yftest uData_post_msg tag =%d\n",tag);
    sensor_msg_pkg_t data_msg;
    memset(&data_msg, 0, sizeof(data_msg));
...
int abs_data_model_init(void)
{
    ...
    for (i = 0; i < ABS_DATA_MAX_CNT; i++) {
        g_sensor_full_info[i].config.mode = DEV_MODE_INV;
    }
    ...
    LOG("yftest abs_data_collect_dev_list g_abs_data_table=%d\n",g_abs_data_table.cnt);
    /*
