...
int udata_service_odr(sensor_tag_e tag)
{
    ...
    switch (tag) {
        ...
        case TAG_DEV_ALS:
        case TAG_DEV_EC:        
        case TAG_DEV_SALINITY:
        case TAG_DEV_TDS:
        case TAG_DEV_WINDSPD:
        case TAG_DEV_WINDDIR:
        case TAG_DEV_RAIN:
        case TAG_DEV_TODAYRAIN:
            odr = 3000;
            break;
#endif
        case TAG_DEV_ACC:
...
{
    int             ret   = 0;
    integer_data_t *pdata = (integer_data_t *)arg;
...
{
    LOG("udata_std_service_init start\n");
    int ret = 0;
    switch (type) {
        case UDATA_SERVICE_ACC:
        ...
        case UDATA_SERVICE_PM25:
            ...
            break;
         case UDATA_SERVICE_PM1P0:
#ifdef AOS_UDATA_SERVICE_PM1P0
            ret = udata_std_service_register(type);
#endif
            break;
         case UDATA_SERVICE_PM10:
#ifdef AOS_UDATA_SERVICE_PM10
            ret = udata_std_service_register(type);
#endif
            break;
...
int service_process_init(void)
{
    ...
    for (i = 0; i < UDATA_MAX_CNT; i++) {
        if (!abs_data_check_dev_tree(udata_service_type_to_tag(i))) {
            LOG("yftest abs_data_check_dev_tree continue\n");
            continue;
        }
