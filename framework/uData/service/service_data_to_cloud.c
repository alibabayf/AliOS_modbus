...
service_pub_info_t g_service_info[UDATA_MAX_CNT] = {
    ...
    { TAG_DEV_RAIN, UDATA_SERVICE_RAIN, 0, DTC_PUBLISH_CYCLE_DEFAULT, false,
      DTC_PARA_NUM_1, NULL },
    { TAG_DEV_TODAYRAIN, UDATA_SERVICE_TODAYRAIN, 0, DTC_PUBLISH_CYCLE_DEFAULT, false,
      DTC_PARA_NUM_1, NULL },
...
static int service_dtc_acc_publish(udata_type_e type, void *data)
{
    ...
    len = snprintf((void *)&smcc_msg_pub[0], sizeof(smcc_msg_pub), "%d.%3d",
                   acc->data[0] / 1000, acc->data[0] % 1000);
    ...
    len = snprintf((void *)&smcc_msg_pub[0], sizeof(smcc_msg_pub), "%d.%3d",
                   acc->data[1] / 1000, acc->data[1] % 1000);
    ret = service_dtc_data_set(
    ...
    len = snprintf((void *)&smcc_msg_pub[0], sizeof(smcc_msg_pub), "%d.%3d",
                   acc->data[2] / 1000, acc->data[2] % 1000);
    ret = service_dtc_data_set(
...
static int service_dtc_mag_publish(udata_type_e type, void *data)
{
    ...
    mag = (mag_data_t *)data;
    len = snprintf((void *)&smcc_msg_pub[0], sizeof(smcc_msg_pub), "%d.%3d",
                   mag->data[0] / 1000, mag->data[0] % 1000);
    ret = service_dtc_data_set(
      (g_service_info[type].name_addr + SERVICE_PUB_NAME_LEN), NULL,
      (void *)&smcc_msg_pub[0]);
    if (0 != ret) {
    ...
    len = snprintf((void *)&smcc_msg_pub[0], sizeof(smcc_msg_pub), "%d.%3d",
                   mag->data[1] / 1000, mag->data[1] % 1000);
    ret = service_dtc_data_set(
      (g_service_info[type].name_addr + SERVICE_PUB_NAME_LEN * 2), NULL,
      (void *)&smcc_msg_pub[0]);
    if (0 != ret) {
    ...
    len = snprintf((void *)&smcc_msg_pub[0], sizeof(smcc_msg_pub), "%d.%3d",
                   mag->data[2] / 1000, mag->data[2] % 1000);
    ret = service_dtc_data_set(
      (g_service_info[type].name_addr + SERVICE_PUB_NAME_LEN * 3), NULL,
      (void *)&smcc_msg_pub[0]);
    if (0 != ret) {
...
static int service_dtc_gyro_publish(udata_type_e type, void *data)
{
    ...
    len  = snprintf((void *)&smcc_msg_pub[0], sizeof(smcc_msg_pub), "%d.%6d",
                   gyro->data[0] / 1000000, gyro->data[0] % 1000000);
    ret  = service_dtc_data_set(
      (g_service_info[type].name_addr + SERVICE_PUB_NAME_LEN), NULL,
      (void *)&smcc_msg_pub[0]);
    if (0 != ret) {
    ...
    len = snprintf((void *)&smcc_msg_pub[0], sizeof(smcc_msg_pub), "%d.%6d",
                   gyro->data[1] / 1000000, gyro->data[1] % 1000000);
    ret = service_dtc_data_set(
      (g_service_info[type].name_addr + SERVICE_PUB_NAME_LEN * 2), NULL,
      (void *)&smcc_msg_pub[0]);
    if (0 != ret) {
    ...
    len = snprintf((void *)&smcc_msg_pub[0], sizeof(smcc_msg_pub), "%d.%6d",
                   gyro->data[2] / 1000000, gyro->data[2] % 1000000);
    ret = service_dtc_data_set(
      (g_service_info[type].name_addr + SERVICE_PUB_NAME_LEN * 3), NULL,
      (void *)&smcc_msg_pub[0]);
    if (0 != ret) {
                                  ...
                                  char *buffer, int length)
{
    if (type == UDATA_SERVICE_NOISE || type == UDATA_SERVICE_HUMI) {
        snprintf(buffer, length, "%u.%u", pdata->data / 10, pdata->data % 10);
    } else if (type == UDATA_SERVICE_HCHO || type == UDATA_SERVICE_TVOC) {
        snprintf(buffer, length, "%u.%u", pdata->data / 1000,
                 pdata->data % 1000);
    } else if (type == UDATA_SERVICE_TEMP) {
        int temp = (int)pdata->data;
        snprintf(buffer, length, "%d.%d", temp / 100,
                 temp >= 0 ? (temp % 100) : ((temp * (-1)) % 100));
    } else if (type == UDATA_SERVICE_PH) {
        snprintf(buffer, length, "%u.%u", pdata->data / 100, pdata->data % 100);
    } 
    else if (type == UDATA_SERVICE_VWC) {
        snprintf(buffer, length, "%u.%u", pdata->data / 100, pdata->data % 10000);
    } 
    else if (type == UDATA_SERVICE_HUMI) {
        snprintf(buffer, length, "%u.%u", pdata->data / 100, pdata->data % 10000);
    } 
    else {
...
static int service_dtc_integer_data_publish(udata_type_e type, void *pdata)
{
    int  ret;
    char smcc_msg_pub[SERVICE_PUB_VALUE_LEN];
    printf("yftest66 service_dtc_integer_data_publish type=%d\n",type);
    if (type > UDATA_MAX_CNT) {
...
int uData_msg_report_publish(sensor_msg_pkg_t *msg)
{
    int         ret = 0;
    udata_pkg_t buf;
    LOG("yftest56 uData_msg_report_publish %d\n",msg->index);
    if (false == service_dtc_is_connect()) {
        printf("service_dtc_is_connect false\n");
        return -1;
    }
    if (false == service_dtc_is_publish(msg->value)) {
        
        printf("service_dtc_is_publish false\n");
        return -1;
    }
    ret = uData_report_publish(msg, &buf);
    if (ret != 0) {
        
        printf("uData_report_publish false\n");
        return -1;
    }
    
    LOG("yftest45 service_dtc_publish %d\n",buf.type);
    ret = service_dtc_publish(buf.type, (void *)&buf.payload[0]);
    if (ret != 0) {
        return -1;
    }
...
int service_dtc_default_name_init(udata_type_e type)
{
    ...
    switch (type) {
        ...
        case UDATA_SERVICE_NOISE: {
            name_num                   = DTC_PARA_NUM_1;
            char *temp[DTC_PARA_NUM_1] = { "NoiseLoudness" };
            ret = service_dtc_name_set(type, temp, name_num);
            break;
        }
        ...
        case UDATA_SERVICE_PM25: {
            name_num                   = DTC_PARA_NUM_1;
            char *temp[DTC_PARA_NUM_1] = { "PM25Value" };
            ret = service_dtc_name_set(type, temp, name_num);
            break;
        }
        case UDATA_SERVICE_PM1P0: {
            name_num                   = DTC_PARA_NUM_1;
            char *temp[DTC_PARA_NUM_1] = { "PM1.0Value" };
            ret = service_dtc_name_set(type, temp, name_num);
            break;
        }
        case UDATA_SERVICE_PM10: {
            name_num                   = DTC_PARA_NUM_1;
            char *temp[DTC_PARA_NUM_1] = { "PM10Value" };
            ret = service_dtc_name_set(type, temp, name_num);
            break;
        }
        case UDATA_SERVICE_CO2: {
            name_num                   = DTC_PARA_NUM_1;
            char *temp[DTC_PARA_NUM_1] = { "CO2Value" };
            ret = service_dtc_name_set(type, temp, name_num);
            break;
        }
        ...
        case UDATA_SERVICE_HCHO: {
            name_num                   = DTC_PARA_NUM_1;
            char *temp[DTC_PARA_NUM_1] = { "HCHOLevel" };
            ret = service_dtc_name_set(type, temp, name_num);
            break;
        }
        ...
        case UDATA_SERVICE_TVOC: {
            name_num                   = DTC_PARA_NUM_1;
            char *temp[DTC_PARA_NUM_1] = { "TVOCLevel" };
            ret = service_dtc_name_set(type, temp, name_num);
            break;
        }
        ...
        case UDATA_SERVICE_SALINITY: {
            name_num                    = DTC_PARA_NUM_1;
            char *vwc[DTC_PARA_NUM_1] = { "SALINITY" };
            ret = service_dtc_name_set(type, vwc, name_num);
            break;
        }
