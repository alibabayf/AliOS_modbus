...
int uData_service_get_payload(sensor_tag_e tag, void **paddr, uint32_t *plen)
{
    ...
    if (unlikely((NULL == paddr) || (NULL == plen))) {
        return -1;
    }
...
{
    if (buf == NULL) {
        printf("uData_get_report_pkg buf NULL\n");
        return -1;
    }
    if (index >= UDATA_MAX_CNT) {
        
        printf("uData_get_report_pkg UDATA_MAX_CNT NULL\n");
        return -1;
    }
...
int uData_service_subscribe(udata_type_e type)
{
    ...
    if (index < 0) {
           LOG("yftest uData_find_service index err\n");
        return -1;
    }
...
int uData_service_register(uData_service_t *service)
{
    ...
    int ret = 0;
    LOG("uData_service_register TAG=%d\n",service->tag);
    if (service == NULL) {
...
{
    int ret;
    LOG("yftest66 uData_data_publish start index=%d\n",index);
    if (g_service_db[index]->subscribe == true) {
        ...
        LOG("yftest66 uData_data_publish  index=%d\n",index);
...
static int uData_service_process(sensor_tag_e tag, void *pdata, uint32_t len)
{
    ...
    for (index = 0; index < g_service_cnt; index++) {
        if ((g_service_db[index]->tag == tag) &&
            ...
            ret_size = g_service_db[index]->service_process_cb(tag, pdata, len);

            if (ret_size != 0) {

                ret = uData_install_report_pkg(index, pdata, ret_size);
                if (unlikely(ret)) {
                    return -1;
                }
