                                  ...
                                  char *buffer, int length)
{
    if (type == UDATA_SERVICE_NOISE || type == UDATA_SERVICE_HUMI) {
        ...
        snprintf(buffer, length, "%d.%d", temp / 10,
                 temp >= 0 ? (temp % 10) : ((temp * (-1)) % 10));
...
static int service_dtc_integer_data_publish(udata_type_e type, void *pdata)
{
    int  ret;
    char smcc_msg_pub[SERVICE_PUB_VALUE_LEN];
    LOG("yftest service_dtc_integer_data_publish start type=%d\n",type);
    ...
    LOG("yftest service_dtc_data_post type=%d\n",type);
    ret = service_dtc_data_post(g_service_info[type].name_addr);
...
int uData_msg_report_publish(sensor_msg_pkg_t *msg)
{
    int         ret = 0;
    udata_pkg_t buf;
    LOG("uData_msg_report_publish MSG value=%d,index=%d\n",msg->value,msg->index);
    if (false == service_dtc_is_connect()) {
        LOG("yftest21\n");
        return -1;
    }
...
int service_dtc_name_set(udata_type_e type, char *src[], int num)
{
    ...
    void *addr;
    int   flag;
    LOG("service_dtc_name_set start type=%d,num=%d\n",type,num);
    ...
    LOG("service_dtc_name_set 1 type=%d,num=%d\n",type,num);
    ...
    LOG("service_dtc_name_set 2 type=%d,num=%d\n",type,num);
    ...
    LOG("service_dtc_name_set 3 type=%d,num=%d\n",type,num);
