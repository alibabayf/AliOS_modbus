...
{
    if (unlikely(NULL == bme280_api.write)) {
        return -1;
    }
                                          ...
                                          uint16_t size)
{
    ...
    ret =
      sensor_spi_read(&bme280_ctx_spi, tx_data, tx_data, size, I2C_OP_RETRIES);
    if (unlikely(0 != ret)) {
        free(tx_data);
        return -1;
    }
                                           ...
                                           uint16_t size)
{
    ...
    ret = sensor_spi_write(&bme280_ctx_spi, tx_data, size, I2C_OP_RETRIES);
...
static int drv_humi_bosch_bme280_i2c_read(uint16_t reg, uint8_t *data,
                                          uint16_t size)
{
    return sensor_i2c_read(&bme280_ctx_i2c, reg, data, size, I2C_OP_RETRIES);
}

static int drv_humi_bosch_bme280_i2c_write(uint16_t reg, uint8_t *data,
                                           uint16_t size)
{
    return sensor_i2c_write(&bme280_ctx_i2c, reg, data, size, I2C_OP_RETRIES);
}
...
static int drv_humi_bosch_bme280_ioctl(int cmd, unsigned long arg)
{
    ...
    switch (cmd) {
        case SENSOR_IOCTL_ODR_SET: {
            ...
            ret         = drv_humi_bosch_bme280_set_odr(arg);
            if (unlikely(ret)) {
                return -1;
            }
        } break;
