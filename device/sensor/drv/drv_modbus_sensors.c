...
int modbus_sensor_read(void *buffer, size_t length, int index)
{
    ...
    LOG("mb_read_holding_reginster  slave=0x%x,addr=0x%x,reg_cnt=0x%x\n", sns->slave,sns->addr,sns->reg_cnt);
    ...
    if (status != MB_EX_NONE) {
        ...
        /*
        rc = modbus_read(data, sns->slave, sns->addr, sns->reg_cnt, sns->response_time);
	if (rc <= 0) {
        
        LOG(" modbus_sensor_read start err=%d\n", rc);
		return rc;
	}
