...
static int drv_als_liteon_ltr553_read(void *buf, size_t len)
{
    ...
    ret = sensor_i2c_read(&ltr553_ctx, LTR553_ALS_DATA1_L, &reg_ch1[0],
                          I2C_DATA_LEN, I2C_OP_RETRIES);
    if (unlikely(ret)) {
        return -1;
    }
    ret = sensor_i2c_read(&ltr553_ctx, LTR553_ALS_DATA1_H, &reg_ch1[1],
                          I2C_DATA_LEN, I2C_OP_RETRIES);
    if (unlikely(ret)) {
        return -1;
    }
    ret = sensor_i2c_read(&ltr553_ctx, LTR553_ALS_DATA0_L, &reg_ch0[0],
                          I2C_DATA_LEN, I2C_OP_RETRIES);
    if (unlikely(ret)) {
        return -1;
    }
    ret = sensor_i2c_read(&ltr553_ctx, LTR553_ALS_DATA0_H, &reg_ch0[1],
                          I2C_DATA_LEN, I2C_OP_RETRIES);
    if (unlikely(ret)) {
        return -1;
    }
...
static int drv_ps_liteon_ltr553_read(void *buf, size_t len)
{
    ...
    ret = sensor_i2c_read(&ltr553_ctx, LTR553_PS_DATA_L, &reg_data[0],
                          I2C_DATA_LEN, I2C_OP_RETRIES);
    if (unlikely(ret)) {
        return -1;
    }
    ret = sensor_i2c_read(&ltr553_ctx, LTR553_PS_DATA_H, &reg_data[1],
                          I2C_DATA_LEN, I2C_OP_RETRIES);
    if (unlikely(ret)) {
        return -1;
    }
...
int drv_als_liteon_ltr553_init(void)
{
    ...
    if (!g_init_bitwise) {
        ret = drv_als_ps_liteon_ltr553_validate_id(
          &ltr553_ctx, LTR553_PART_ID_VAL, LTR553_MANUFAC_ID_VAL);
        if (unlikely(ret)) {
            return -1;
        }
    }
