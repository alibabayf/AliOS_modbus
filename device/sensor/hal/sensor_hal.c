...
static int     sensor_poll(file_t *f, bool setup, poll_notify_t notify,
...
static int sensor_set_sensor_service(SENSOR_IRQ_CALLBACK func)
{
    ...
    if (unlikely(g_sensor_irq_cb != NULL)) {
        return 0;
    }
...
{
    if (unlikely(index >= g_sensor_cnt)) {
        return -1;
    }
...
static void sensor_irq_handle(void *arg)
{
    ...
    if (unlikely(index >= TAG_DEV_SENSOR_NUM_MAX)) {
        return;
    }

    if (unlikely((g_sensor_obj[index]->mode != DEV_FIFO) &&
...
static int sensor_register_irq(int index)
{
    ...
    if (unlikely(0 == g_sensor_obj[index]->data_len)) {
        return -1;
    }

    ret =
      hal_gpio_enable_irq(&(g_sensor_obj[index]->gpio), IRQ_TRIGGER_RISING_EDGE,
...
static int load_sensor_config(int index)
{
    ...
    if (unlikely(g_sensor_obj[index] == NULL)) {
...
int sensor_create_obj(sensor_obj_t *sensor)
{
    ...
    if (unlikely(g_sensor_obj[g_sensor_cnt] == NULL)) {
        return -1;
    }
...
static int sensor_hal_get_dev_list(void *buf)
{
    ...
    if (unlikely(list->cnt >= TAG_DEV_SENSOR_NUM_MAX)) {
        return -1;
    }
...
{
    int index = 0;
    LOG("yftest sensor_open start\n");
    if (unlikely((node == NULL) || (file == NULL))) {
        return -1;
    }
    ...
    if ((g_sensor_obj[index]->open == NULL) || (index < 0)) {
        ...
        LOG("yftest find_selected_sensor err\n");
        return -1;
    }
    LOG("yftest find_selected_sensor index=%d\n",index);
...
static int sensor_close(file_t *file)
{
    ...
    if (strncmp(sensor_node_path, (file->node->i_name),
                strlen(file->node->i_name)) == 0) {
        return 0;
    }
...
static ssize_t sensor_read(file_t *f, void *buf, size_t len)
{
    ...
    if (unlikely((g_sensor_obj[index]->read == NULL) || (index < 0))) {
        goto error;
    }
