...
static void nrf_dfu_invalidate_bank(nrf_dfu_bank_t * p_bank, uint8_t app_flag)
{
    ...
    if (app_flag == 0)
        ...
        s_dfu_settings.bank_app_current = NRF_DFU_CURRENT_BANK_0;
}
 ...
 */
static uint32_t nrf_dfu_bank_continue(uint32_t src_addr, uint32_t dst_addr, uint8_t app_flag)
{
    ...
    uint32_t ret_val     = 0;
    uint32_t target_addr = dst_addr + s_dfu_settings.write_offset;
    uint32_t length_left;
    uint32_t cur_len;
    uint32_t crc;
    uint32_t i = 0;

    nrf_dfu_bank_t *bank0, *bank1;

    if (app_flag)
    {
        bank0 = &(s_dfu_settings.bank_app_0);
        bank1 = &(s_dfu_settings.bank_app_1);
    }
    else
    {
        bank0 = &(s_dfu_settings.bank_0);
        bank1 = &(s_dfu_settings.bank_1);
    }

    image_size = bank1->image_size;
    length_left = (image_size - s_dfu_settings.write_offset);

    src_addr += s_dfu_settings.write_offset;

    printf("In nrf_dfu_bank_continue, src_addr is 0x%x, dst_add is 0x%x.\r\n", src_addr, dst_addr);

    if (app_flag) printf("Will copy APP bank.\r\n");
    ...
    crc = crc32_compute((uint8_t*)dst_addr, image_size, NULL);

    printf("In %s line %d, the crc calculated is 0x%x, the expected crc is 0x%x.\r\n", \
           __func__, __LINE__, crc, bank1->image_crc);

    if (crc == bank1->image_crc)
    {
        printf("Good, calculated crc and read crc is equal!\r\n");
        bank0->bank_code = NRF_DFU_BANK_VALID_APP;
        bank0->image_crc = crc;
        bank0->image_size = image_size;
    }
    else
    {
    ...
    nrf_dfu_invalidate_bank(bank1, app_flag);
    ...
    switch (p_bank->bank_code)
    {
       case NRF_DFU_BANK_VALID_APP:
           if(s_dfu_settings.bank_current == NRF_DFU_CURRENT_BANK_1)
           {
               ...
               ret_val = nrf_dfu_bank_continue(src_addr, MAIN_APPLICATION_START_ADDR, 0);
           }
...
static uint32_t nrf_dfu_continue()
{
    ...
    if (s_dfu_settings.bins_flag == 1) { // bins path
        uint32_t src = OTA_OFFSET, dst = 0;
        uint8_t app_flag = 0;

        printf("Multiple BINs layout detected.\r\n");

        if (s_dfu_settings.bank_layout == NRF_DFU_BANK_LAYOUT_SINGLE) {
            printf("Single bank OTA is not supported.\r\n");
            return 0;
        }

        if (s_dfu_settings.bank_app_current == NRF_DFU_CURRENT_BANK_1 && \
            s_dfu_settings.bank_app_1.bank_code == NRF_DFU_BANK_VALID_APP) {
            printf("Will update app bank.\r\n");
            dst = BIN2_OFFSET;
            app_flag = 1;
        } else if (s_dfu_settings.bank_current == NRF_DFU_CURRENT_BANK_1 && \
                   s_dfu_settings.bank_1.bank_code == NRF_DFU_BANK_VALID_APP) {
            printf("Will update kernel bank.\r\n");
            dst = BIN_OFFSET;
            app_flag = 0;
        }

        if (dst != 0) ret_val = nrf_dfu_bank_continue(src, dst, app_flag);

        return ret_val;
    } else { // single bin path
        nrf_dfu_bank_t    * p_bank;
        uint32_t            src_addr = MAIN_APPLICATION_START_ADDR;

        printf("Single BIN layout detected.\r\n");
